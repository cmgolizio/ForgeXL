...
reported as *missing* `Sales Person` and as *carrying* the other name, with
...
as *metadata on the version*, never written into the rows.
...
- **number** marks a column whose arrival as *text* is worth reporting
...
A blank rep on a *transaction* is not a defect — ownership comes from the
snapshot. A blank rep in a *snapshot* is a refusal, because it leaves the one
...
- **Snapshot**: the month is supplied by the caller, and identical bytes for a
  *different* month are normal — account ownership often does not change, so
...
10F exists to prevent. When the same *file* is re-uploaded both rules apply, and
