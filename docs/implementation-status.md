...
3. **No fastexcel option avoids it.** `dtype_coercion="strict"` and
   ...
   `dtypes="string"` would preserve everything but would make *every* numeric
   column text.
...
between *refuse*, *warn* and *fall back* is a product decision, not an
...
it states the *correct* behaviour for a mixed numeric/text column read from a
...
65. **An Excel upload silently destroys a column that mixes numbers and text.**
    ...
    - **No fastexcel setting avoids it.** `dtype_coercion="strict"` and
      `schema_sample_rows=None` were both measured and both still coerce.
      `dtypes="string"` preserves everything but makes *every* numeric column
      text.
    ...
    *refuse the upload*, *warn in the manifest* and *fall back to openpyxl* is
...
47. **The fixture system is a Python module, not a directory of spreadsheets.**
    ...
    *builds* workbooks rather than workbooks. A test asserts the repository
...
49. **One `xfail(strict=True)` in the suite.** The project has never used one
    ...
    This is the case the rule does not cover: the test asserts the *correct*
...
- **`test_contract_freeze.py` is byte-identical to its 6F state** and 6I has no
  reason to change it. If `FROZEN_ROUTES` needs an edit during a *cleanup*
...
never going to — it is the phase that makes everything *except* those two
